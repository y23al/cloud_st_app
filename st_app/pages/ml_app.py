...
with st.form("入力"):
    ...
    with col1_2:
        avg_hour = st.number_input("1日平均SNS使用時間",
    ...
    with col2_1:
        ...
        country = st.selectbox("国",
                               cat_values_dict["Country"], index=jp_idx)
    with col2_2:
        platform = st.selectbox("最もよく使用するSNS",
                                cat_values_dict["Most_Used_Platform"],
                                index=None)
    with col2_3:
        academic_level = st.radio("現在の所属",
                                  ["High School", "Undergraduate", "Graduate"],
                                  index=None)
    with col3_1:
        sleep = st.number_input("1日の平均睡眠時間",
                                  min_value=0., max_value=24., value=8)
    with col3_2:
        conflicts = st.number_input("SNSに関する家族や友人などと言い争いの数",
                                   min_value=0, value=0)
    with col3_3:
        relationship = st.radio("交際状況",
